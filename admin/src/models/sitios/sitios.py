...
from geoalchemy2 import (
    elements as geoelements,
    functions as geo_functions,
    Geometry,
    Geography,
)
...
def sitio_create(**kwargs):
    ...
    if "estado_conservacion" in kwargs:
        kwargs["estado_conservacion"] = EstadoConservacion(
            kwargs["estado_conservacion"]
        )
...
def sitio_exists(nombre, ciudad):
    ...
    existing = (
        ...
        .filter(SitioHistorico.nombre == nombre, SitioHistorico.ciudad == ciudad)
        .first()
    )
...
def sitio_index(page=1, per_page=25, filters=None):
    ...
    if filters:
        ...
        if filters.get("categoria"):
            query = query.filter(
                SitioHistorico.categoria == Categoria(filters["categoria"])
            )
        ...
        if filters.get("estado_conservacion"):
            query = query.filter(
                ...
                == EstadoConservacion(filters["estado_conservacion"])
            )
    ...
    # Aplicar orden
    order_by = (
        filters.get("order_by", "fecha_registro") if filters else "fecha_registro"
    )
...
def sitio_show(id):
    """Obtener un sitio histórico por ID y registrar visita"""
...
def sitio_update(id, **kwargs):
    ...
    valores_anteriores = {
        ...
        "estado_conservacion": (
            sitio.estado_conservacion.value if sitio.estado_conservacion else None
        ),
    ...
    if "estado_conservacion" in kwargs:
        nuevo_estado = (
            kwargs["estado_conservacion"].value
            if kwargs["estado_conservacion"]
            else None
        )
    ...
    if tags_changed:
        registrar_evento_historial(
            sitio_id=id, tipo_accion="Cambio de tags", detalles=f"Tags actualizados"
        )
...
def sitio_get_coordinates(sitio):
    ...
    result = (
        ...
        .filter(SitioHistorico.id == sitio.id)
        .first()
    )
...
def sitio_count_by_estado():
    ...
    return (
        db.session.query(
            SitioHistorico.estado_conservacion, func.count(SitioHistorico.id)
        )
        .group_by(SitioHistorico.estado_conservacion)
        .all()
    )
...
def sitio_count_visible():
    ...
    return (
        db.session.query(func.count(SitioHistorico.id))
        .filter(SitioHistorico.visible == True)
        .scalar()
    )
...
def obtener_sitios(
    ...
    match order_by:
        ...
        case "oldest":
            query = query.order_by(SitioHistorico.fecha_registro.asc())
        case "rating_asc":
            query = query.order_by(SitioHistorico.calificacion_promedio.asc())
        case "rating_desc":
            query = query.order_by(SitioHistorico.calificacion_promedio.desc())
        case "visits_asc":
            query = query.order_by(SitioHistorico.visitas.asc())
        case "visits_desc":
...
def get_search_options():
    ...
    provincias = (
        db.session.query(SitioHistorico.provincia)
        .distinct()
        .order_by(SitioHistorico.provincia)
        .all()
    )
    ...
    ciudades = (
        db.session.query(SitioHistorico.ciudad)
        .distinct()
        .order_by(SitioHistorico.ciudad)
        .all()
    )
...
def sitio_incrementar_visita(id):
    """Incrementa el contador de visitas de un sitio histórico"""
    sitio = db.session.get(SitioHistorico, id)
    if sitio:
        sitio.visitas = (sitio.visitas or 0) + 1
        db.session.commit()
    return sitio
