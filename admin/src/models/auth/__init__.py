...
def user_paginate(page=1, filter_type="mail", date_order="desc"):
    ...
    match filter_type:
        ...
        case "admin":
            users_query = users_query.filter(user.role.has(name="admin")).order_by(
                user.id.asc()
            )
        case "editor":
            users_query = users_query.filter(user.role.has(name="editor")).order_by(
                user.id.asc()
            )
        case "active":
            users_query = users_query.filter(user.enabled == True).order_by(
                user.id.asc()
            )
        case "inactive":
            users_query = users_query.filter(user.enabled == False).order_by(
                user.id.asc()
            )
...
def create_permission(name):
    if not permiso_valido(name):
        raise ValueError(
            f"❌ Permission '{name}' no permitido (debe existir en Enum Permisos)"
        )
